...
void Write_Bitmap(FILE* fptr, BITMAP4* bm, int nx, int ny, int format) {
    ...
    case 13:
        ...
        if(ABS(format) == 12 || ABS(format) == 13) putc(10, fptr); /* compressed RGB */
        else
    ...
    for(j = 0; j < ny; j++) {
        if(format > 0) rowindex = j * nx;
        else
            rowindex = (ny - 1 - j) * nx;
        switch(ABS(format)) {
        case 12: WriteTGACompressedRow(fptr, &(bm[rowindex]), nx, 3); break;
        case 13: WriteTGACompressedRow(fptr, &(bm[rowindex]), nx, 4); break;
        }
        for(i = 0; i < nx; i++) {
            if(format > 0) index = rowindex + i;
            else
                index = rowindex + i;
            switch(ABS(format)) {
            case 1:
            ...
            case 9:
                ...
                if(ABS(format) == 11) putc(bm[index].a, fptr);
                break;
            case 2:
            case 3:
            case 5:
            case 8:
                ...
                putc(bm[index].b, fptr);
                break;
            case 4: putc((bm[index].r + bm[index].g + bm[index].b) / 3, fptr); break;
            case 6:
    ...
    for(;;) {
        ...
        if(!readytowrite) {
            if(Same_BitmapPixel(currentpixel, nextpixel)) {
                if(packettype == 0) {
                    counter++;
                    if(counter >= 128 || (pixelstart + counter) >= width) readytowrite = TRUE;
                } else {
                    counter--;
                    readytowrite = TRUE;
                }
        ...
        if(readytowrite) {
            ...
            if(packettype == 0) {
                ...
                for(i = 0; i < counter; i++) {
                    ...
                    if(depth == 4) putc(bm[pixelstart + i].a, fptr);
                }
            }
            if((pixelstart = pixelstart + counter) >= width) break; /* From for (;;) */
            readytowrite = FALSE;
            packettype = 0;
            counter = 1;
        }
...
*/
void BiCubicScale(BITMAP4* bm_in, int nx, int ny, BITMAP4* bm_out, int nnx, int nny) {
...
double BiCubicR(double x) {
    ...
    if(xp2 > 0) r += xp2 * xp2 * xp2;
    if(xp1 > 0) r -= 4 * xp1 * xp1 * xp1;
    if(x > 0) r += 6 * x * x * x;
    if(xm1 > 0) r -= 4 * xm1 * xm1 * xm1;
...
*/
void GaussianScale(BITMAP4* bm_in, int nx, int ny, BITMAP4* bm_out, int nnx, int nny, double r) {
    int i, j, ii, jj, ci, cj;
    long index;
...
int Draw_Marker(BITMAP4* bm, int nx, int ny, int cx, int cy, BITMAP4 col, int type, int radius) {
    ...
    case 1: // Filled circle
        for(i = -radius; i <= radius; i++) {
            for(j = -radius; j <= radius; j++) {
                if(sqrt(i * i + (double)j * j) > radius) continue;
                x = cx + i;
                y = cy + j;
                Draw_Pixel(bm, nx, ny, x, y, col);
            }
...
int Draw_Box(BITMAP4* bm, int nx, int ny, int x1, int y1, int x2, int y2, BITMAP4 col) {
    ...
    for(i = x1; i < x2; i++)
        for(j = y1; j < y2; j++) Draw_Pixel(bm, nx, ny, i, j, col);
...
BITMAP4 Get_Pixel(BITMAP4* bm, int nx, int ny, int x, int y) {
    ...
    if(x < 0 || y < 0 || x >= nx || y >= ny) return (black);
    index = y * nx + x;
    return (bm[index]);
}
...
int IsTGA(char* fname) {
    ...
    if(strstr(s, ".tga") != NULL) return (TRUE);
    if(strstr(s, ".TGA") != NULL) return (TRUE);
...
void TGA_Info(FILE* fptr, int* width, int* height, int* depth) {
    ...
    *depth = 0;
    ...
    if(fread(&header.colourmaporigin, 2, 1, fptr) != 1) return;
    if(fread(&header.colourmaplength, 2, 1, fptr) != 1) return;
    header.colourmapdepth = fgetc(fptr);
    if(fread(&header.x_origin, 2, 1, fptr) != 1) return;
    if(fread(&header.y_origin, 2, 1, fptr) != 1) return;
    lo = fgetc(fptr);
    hi = fgetc(fptr);
...
int TGA_Read(FILE* fptr, BITMAP4* image, int* width, int* height) {
    ...
    header.colourmapdepth = fgetc(fptr);
    if(fread(&header.x_origin, 2, 1, fptr) != 1) return (10);
    if(fread(&header.y_origin, 2, 1, fptr) != 1) return (10);
    lo = fgetc(fptr);
    hi = fgetc(fptr);
    header.width = hi * 256 + lo;
    *width = header.width;
    ...
    */
    if(header.colourmaptype != 0 && header.colourmaptype != 1) { return (3); }
    ...
    if(header.datatypecode == 1) {
        ...
        for(i = 0; i < header.colourmaplength; i++) {
            if((int)fread(p, 1, bytes2read, fptr) != bytes2read) return (5);
    ...
    while(n < header.width * header.height) {
        if(header.datatypecode == 1) { /* Indexed uncompressed */
            if((index = fgetc(fptr)) == EOF) return (4);
            if(index < 0) index = 0;
            if(index >= header.colourmaplength) index = header.colourmaplength - 1;
            image[n] = ctable[index];
            n++;
        } else if(header.datatypecode == 2) { /* RGB Uncompressed */
            if((int)fread(p, 1, bytes2read, fptr) != bytes2read) return (4);
            ...
            if((int)fread(p, 1, bytes2read + 1, fptr) != bytes2read + 1) return (4);
            j = p[0] & 0x7f;
            ...
            if(p[0] & 0x80) { /* RLE chunk */
                ...
                for(i = 0; i < j; i++) {
                    if((int)fread(p, 1, bytes2read, fptr) != bytes2read) return (6);
                    TGA_MergeBytes(&(image[n]), p, bytes2read);
                    n++;
                }
            }
...
int IsPPM(char* fname) {
    ...
    if(strstr(s, ".ppm") != NULL) return (TRUE);
    if(strstr(s, ".PPM") != NULL) return (TRUE);
...
int PPM_Info(FILE* fptr, int* width, int* height, int* depth) {
    ...
    *depth = 0;
    ...
    // Magic number
    if(fgets(s, 254, fptr) == NULL) return (FALSE);
    ...
    if(sscanf(s, "%d %d", width, height) != 2) return (FALSE);

    // Maximum value
    if(fgets(s, 254, fptr) == NULL) return (FALSE);
    if(sscanf(s, "%d", depth) != 1) return (FALSE);
...
int PPM_Read(FILE* fptr, COLOUR16* img, int* width, int* height, int* depth) {
    ...
    for(j = 0; j < (*height); j++) {
        for(i = 0; i < (*width); i++) {
            ...
            if(!Read_UShort(fptr, &r, TRUE)) return (FALSE);
            if(!Read_UShort(fptr, &g, TRUE)) return (FALSE);
            if(!Read_UShort(fptr, &b, TRUE)) return (FALSE);
            img[index].r = r;
            img[index].g = g;
            img[index].b = b;
        }
...
int PPM_Write(FILE* fptr, COLOUR16* img, int width, int height, int depth) {
    ...
    for(j = height - 1; j >= 0; j--) {
        for(i = 0; i < width; i++) {
            index = j * width + i;
            if(!Write_UShort(fptr, img[index].r, TRUE)) return (FALSE);
            if(!Write_UShort(fptr, img[index].g, TRUE)) return (FALSE);
            if(!Write_UShort(fptr, img[index].b, TRUE)) return (FALSE);
        }
    }
...
int BMP_Read(FILE* fptr, BITMAP4* image, size_t* width, size_t* height) {
    ...
    // Read information header
    if(fread(&infoheader, sizeof(INFOHEADER), 1, fptr) != 1) { return (1); }
    *width = ABS(infoheader.width);
    *height = ABS(infoheader.height); // Supposedly supports image flipping
    ...
    if(infoheader.ncolours > 0) {
        for(size_t i = 0; i < (size_t)infoheader.ncolours; i++) {
            ...
            if(fread(&colourindex[i].junk, sizeof(unsigned char), 1, fptr) != 1) return (1);
        }
        gotindex = TRUE;
    }
    ...
    for(size_t j = 0; j < (*height); j++) {
        for(size_t i = 0; i < (*width); i++) {
            ...
            case 4: break;
            case 8:
                if(fread(&grey, sizeof(unsigned char), 1, fptr) != 1) return (2);
                if(gotindex) {
            ...
            case 24:
            case 32:
                if(fread(&b, sizeof(unsigned char), 1, fptr) != 1) return (2);
                if(fread(&g, sizeof(unsigned char), 1, fptr) != 1) return (2);
                if(fread(&r, sizeof(unsigned char), 1, fptr) != 1) return (2);
                if(infoheader.bits == 32) {
                    if(fread(&a, sizeof(unsigned char), 1, fptr) != 1) return (2);
                }
                image[j * (*width) + i].r = r;
                image[j * (*width) + i].g = g;
                image[j * (*width) + i].b = b;
    ...
    if(flip) Flip_Bitmap(image, (*width), (*height), 0);
...
int IsRAW(char* fname) {
    ...
    if(strstr(s, ".raw") != NULL) return (TRUE);
    if(strstr(s, ".rgb") != NULL) return (TRUE);
    if(strstr(s, ".RAW") != NULL) return (TRUE);
    if(strstr(s, ".RGB") != NULL) return (TRUE);
...
int RAW_Read(FILE* fptr, COLOUR16* image, int w, int h, int swap) {
    ...
    for(j = 0; j < h; j++) {
        for(i = 0; i < w; i++) {
            index = (h - 1 - j) * w + i;
            if(!Read_UShort(fptr, &r, swap)) return (FALSE);
            if(!Read_UShort(fptr, &g, swap)) return (FALSE);
            if(!Read_UShort(fptr, &b, swap)) return (FALSE);
            image[index].r = r;
            image[index].g = g;
            image[index].b = b;
        }
...
int Read_UShort(FILE* fptr, short unsigned* n, int swap) {
    ...
    if(fread(n, 2, 1, fptr) != 1) return (FALSE);
    if(swap) {
...
int Write_UShort(FILE* fptr, short unsigned n, int swap) {
    ...
    if(!swap) {
        if(fwrite(&n, 2, 1, fptr) != 1) return (FALSE);
    } else {
        cptr = (unsigned char*)(&n);
        tmp = cptr[0];
        cptr[0] = cptr[1];
...
boolean IsJPEG(const char* fname) {
    ...
    if(strstr(s, ".jpg") != NULL) return (TRUE);
    if(strstr(s, ".jpeg") != NULL) return (TRUE);
    if(strstr(s, ".JPG") != NULL) return (TRUE);
    if(strstr(s, ".JPEG") != NULL) return (TRUE);
...
int JPEG_Write(FILE* fptr, BITMAP4* image, int width, int height, int quality) {
    ...
    if((jimage = malloc(width * 3)) == NULL) return (1);
    ...
    while(cinfo.next_scanline < cinfo.image_height) {
        for(i = 0; i < width; i++) {
            if(flip) index = (height - 1 - j) * width + i;
            else
                index = j * width + i;
...
int JPEG_Info(FILE* fptr, int* width, int* height, int* depth) {
    ...
    while(cinfo.output_scanline < cinfo.output_height) jpeg_read_scanlines(&cinfo, (JSAMPARRAY)buffer, 1);
...
int JPEG_Read(FILE* fptr, BITMAP4* image, int* width, int* height) {
    ...
    *height = cinfo.output_height;
    ...
    row_stride = cinfo.output_width * cinfo.output_components;
    if((buffer = malloc(row_stride * sizeof(JSAMPLE))) == NULL) return (2);
...
boolean IsPNG(const char* fname) {
    ...
    if(strstr(s, ".png") != NULL) return (TRUE);
    if(strstr(s, ".PNG") != NULL) return (TRUE);
...
int PNG_Write(FILE* fptr, const BITMAP4* image, int width, int height, int flip) {
    ...
    for(y = 0; y < height; y++) {
        png_byte* row = row_pointers[y];
        for(x = 0; x < width; x++) {
            if(!flip) index = (height - 1 - y) * width + x;
            else
                index = y * width + x;
            png_byte* ptr = &(row[x * 4]);
            ptr[0] = image[index].r;
...
int IsTIFF(char* fname) {
    ...
    if(strstr(s, ".tif") != NULL) return (TRUE);
    if(strstr(s, ".TIF") != NULL) return (TRUE);
    if(strstr(s, ".tiff") != NULL) return (TRUE);
    if(strstr(s, ".TIFF") != NULL) return (TRUE);
...
int TIFF_Write(char* fname, BITMAP4* image, int width, int height) {
    ...
    if((tptr = TIFFOpen(fname, "w")) == NULL) return (FALSE);
    ...
    for(row = 0; row < height; row += rowsperstrip) {
        if(row + rowsperstrip > height) rows_to_write = height - row;
        else
...
int TIFF_Read16(char* fname, COLOUR16* image) {
    ...
    if((tptr = TIFFOpen(fname, "r")) == NULL) return (FALSE);
...
int TIFF_Write16(char* fname, COLOUR16* image, int width, int height) {
    ...
    for(row = 0; row < height; row++) TIFFWriteEncodedStrip(tptr, row, &(image[row * width]), 3 * 2 * width);
...
int TIFF_Read(char* fname, BITMAP4* image) {
    ...
    if((raster = (uint32*)_TIFFmalloc(w * h * sizeof(uint32))) == NULL) return (FALSE);
...
int IsEXR(char* fname) {
    ...
    if(strstr(s, ".exr") != NULL) return (TRUE);
    if(strstr(s, ".EXR") != NULL) return (TRUE);
...
int EXR_Info(char* fname, int* width, int* height) {
    ...
    if((infile = ImfOpenInputFile(fname)) == NULL) return (FALSE);
